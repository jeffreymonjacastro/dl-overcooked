...
class RecipeAwareGreedyPolicy(GreedyFullTaskPolicy):
    """Greedy policy that respects mixed onion/tomato recipes."""

    def __init__(self, avoid_teammate: bool = True, seed: int | None = None):
    ...
    def _choose_target(self, state) -> tuple[int, int] | None:
        ...
        if held is not None:
            ...
            if held.name == "dish":
                ...
                if ready_pots:
                    return self._nearest(player.position, ready_pots)
                almost_ready = list(pot_states.get("cooking", [])) + list(pot_states.get(f"{Recipe.MAX_NUM_INGREDIENTS}_items", []))
                return self._nearest(player.position, almost_ready)
            if held.name in {"onion", "tomato"}:
                return self._nearest(player.position, self._pots_that_can_accept_named_ingredient(state, pot_states, held.name))
            return None
        ...
        if ready_pots:
            ...
            if counter_dishes:
                return self._nearest(player.position, counter_dishes)
            return self._nearest(player.position, mdp.get_dish_dispenser_locations())

        desired = self._desired_ingredient(state, pot_states)
        if desired is not None:
            counter_ingredients = self._counter_objects_by_name(state, desired)
            if counter_ingredients and self._pots_that_can_accept_named_ingredient(state, pot_states, desired):
                return self._nearest(player.position, counter_ingredients)
            dispensers = self._ingredient_dispenser_locations_for(desired)
            if dispensers:
    ...
    def _ingredient_dispenser_locations_for(self, ingredient: str) -> list[tuple[int, int]]:
        if ingredient == "tomato":
            return list(self.mdp.get_tomato_dispenser_locations())
        return list(self.mdp.get_onion_dispenser_locations())

    def _desired_ingredient(self, state, pot_states) -> str | None:
        recipes = self._recipes()
        in_progress = []
        for pos in self._pots_that_can_accept_ingredients(state, pot_states):
            ingredients = self._pot_ingredients(state, pos)
            in_progress.append((pos, ingredients))
        in_progress.sort(key=lambda item: (-len(item[1]), self._nearest_distance(state.players[self.agent_index].position, [item[0]])))
        for _, ingredients in in_progress:
            needed = self._next_needed_ingredients(ingredients, recipes)
            if needed:
                # With a greedy onion partner, favor tomato when it is useful.
                if "tomato" in needed:
                    return "tomato"
                return needed[0]
        return None

    def _pots_that_can_accept_named_ingredient(self, state, pot_states, ingredient: str) -> list[tuple[int, int]]:
        recipes = self._recipes()
        candidates = []
        for pos in self._pots_that_can_accept_ingredients(state, pot_states):
            ingredients = self._pot_ingredients(state, pos)
            if ingredient in self._next_needed_ingredients(ingredients, recipes):
                candidates.append(pos)
        return candidates

    def _recipes(self) -> list[tuple[str, ...]]:
        config = getattr(self.mdp, "recipe_config", {}) or {}
        orders = config.get("all_orders") or getattr(self.mdp, "start_all_orders", None) or []
        recipes = [tuple(order.get("ingredients", ())) for order in orders if order.get("ingredients")]
        recipes.sort(key=lambda recipe: (len(recipe), recipe.count("onion")))
        return recipes or [("onion", "onion", "onion")]

    def _next_needed_ingredients(self, current: list[str], recipes: list[tuple[str, ...]]) -> list[str]:
        current_counts = Counter(current)
        possible = []
        for recipe in recipes:
            recipe_counts = Counter(recipe)
            if any(current_counts[name] > recipe_counts[name] for name in current_counts):
                continue
            if sum(current_counts.values()) >= len(recipe):
                continue
            for ingredient in ("tomato", "onion"):
                if current_counts[ingredient] < recipe_counts[ingredient]:
                    possible.append((len(recipe), ingredient))
        possible.sort()
        return [ingredient for _, ingredient in possible]

    @staticmethod
    def _pot_ingredients(state, pos: tuple[int, int]) -> list[str]:
        obj = state.objects.get(pos)
        if obj is None or obj.name != "soup":
            return []
        return list(getattr(obj, "ingredients", []) or [])

    @staticmethod
    def _nearest_distance(origin: tuple[int, int], positions: Iterable[tuple[int, int]]) -> int:
        positions = list(positions)
        if not positions:
            return 10**9
        return min(abs(p[0] - origin[0]) + abs(p[1] - origin[1]) for p in positions)


class HybridOfficialScorePolicy(Agent):
    """Hard router between the strongest built-in state policies found so far.

    This policy is intentionally state-based and lives outside StudentAgent,
    because StudentAgent receives only featurized observations. The guide asks
    for score-first routing; the rules below are selected from measured rollout
    matrices, not validation loss.
    """
    ...
    def _context(self, state):
        ...
        return layout, partner

    def _select_policy(self, state):
        layout, partner = self._context(state)
        if layout == "cramped_room":
            if partner == "random_motion":
                return "greedy_full_task", self.greedy_local
            return "greedy_human_model", self.greedy_human
        if layout == "coordination_ring":
            if partner == "greedy_full_task_noise_015":
                return "greedy_human_model", self.greedy_human
            return "greedy_full_task", self.greedy_local
        return "greedy_full_task", self.greedy_local

    def action(self, state):
        mode, policy = self._select_policy(state)
        action, info = policy.action(state)
        info = dict(info or {})
        info["policy_name"] = "hybrid_official_score"
        info["selected_mode"] = mode
        return action, info
